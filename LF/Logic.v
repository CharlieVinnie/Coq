...
Proof.
  intros n m H.
  assert (n=0) as H1. { destruct n. reflexivity. discriminate H. }
  assert (m=0) as H2. { destruct m. reflexivity. rewrite <- plus_n_Sm in H. discriminate H. }
  apply (conj H1 H2).
Qed.
(** [] *)
...
Proof.
  ...
  - left. reflexivity.
  - simpl in H. apply and_exercise in H. destruct H as [ Hm _ ]. right. apply Hm.
Qed.
(** [] *)
...
Theorem not_implies_our_not : forall (P:Prop),
  ~ P -> (forall (Q:Prop), P -> Q).
Proof.
  intros P H1 H2 H3. unfold not in H1. specialize (H1 H3) as H'. destruct H'.
Qed.
(** [] *)
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  unfold not. intros P Q H1 H2 H3. apply H2. apply H1. apply H3.
Qed.
(** [] *)
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  unfold not. intros P [H1 H2]. apply H2. apply H1.
Qed.
(** [] *)
...
Proof.
  ...
  - intros [Hp | Hqr]. split.
    ...
    + destruct Hqr as [Hq Hr]. split. * right. apply Hq. * right. apply Hr.
...
Proof.
  ...
  - intros [H1 | H2].
    + destruct H1 as [x Px]. exists x. left. apply Px.
    + destruct H2 as [x Qx]. exists x. right. apply Qx.
Qed.
(** [] *)

Lemma leb_n_Sm : forall n m, (n<=?m)=true -> (n<=?S m)=true.
Proof.
  induction n as [ | n IHn ].
  - intros m H. reflexivity.
  - intros m H. simpl. destruct m as [ | m ].
    + discriminate H.
    + simpl in H. specialize (IHn m). apply (IHn H).
Qed.
...
Proof.
  ...
  intros n m H. generalize dependent m. induction n as [ | n' IHn' ].
  - intros m. exists m. reflexivity.
  - intros m H. destruct m as [ | m'].
    + discriminate H.
    + simpl. simpl in H. specialize (IHn' m' H) as H'.
      destruct H' as [x E]. exists x. f_equal. apply E.
Qed.
(*
  intros n m H. induction m as [ | m IHm ].
  - destruct n. + exists 0. reflexivity. + discriminate H.
  - destruct (n =? S m) eqn: E.
    + specialize (eqb_true n (S m) E) as eq.
      rewrite eq. exists 0. apply plus_n_O.
    + destruct (n=?m) eqn: Enm.
      * specialize (eqb_true n m Enm) as Eqnm. rewrite Eqnm. exists 1.
        rewrite <- plus_n_Sm. rewrite <- plus_n_O. reflexivity.
      * 


destruct (n<=?m) eqn: nm.
    + assert (true=true) as qwq. reflexivity.
      specialize (IHm qwq). destruct IHm as [x E].
      exists (S x). rewrite <- plus_n_Sm. rewrite E. reflexivity.
    + exists 0. rewrite <- plus_n_O.
      generalize dependent m. intro
      destruct n as [ | n' ].
      * discriminate nm.
      * f_equal. simpl in H. unfold leb in H.
*)

Theorem plus_exists_leb : forall n m, (exists x, m = n+x) -> n <=? m = true.
Proof.
  induction n as [ | n IHn' ].
  - intros m [x E]. reflexivity.
  - intros m [x E]. destruct m as [ | m']. discriminate E.
    simpl in E. injection E as E.
    (*specialize (IHn' m' [x E]).*)
    simpl. apply IHn'. exists x. apply E.
Qed.
...
Proof.
  ...
  { induction l as [|x l' IHl'].
    ...
    - simpl. intros [H|H].
      ...
      + apply IHl' in H. destruct H as [x' [H1 H2]]. exists x'. split. apply H1. right. apply H2. }
  { intros [x [H1 H2]]. induction l as [ | x' l' IHl' ].
    - unfold In in H2. destruct H2.
...
Theorem All_In :
  forall T (P : T -> Prop) (l : list T),
    ...
    All P l.
Proof.
  intros T P. induction l as [|x l' IH].
  - simpl. split.
    + intros x. apply I.
    + intros _ x [].
  - simpl. split.
    ...
    + intros [H1 H2] x' [H3|H4].
      * rewrite <- H3. apply H1.
      * rewrite <- IH in H2. apply H2. apply H4.
Qed.
(** [] *)
...
(*
  induction n as [ | n' IH ] eqn: E.
  - simpl. exists 0. reflexivity.
  - specialize (IH E).
*)
  induction n as [ | n IH ].
  - simpl. exists 0. reflexivity.
  - rewrite even_S. destruct (even n) eqn: Ev.
    + simpl. destruct IH as [k E]. exists k. rewrite E. reflexivity.
    + simpl. destruct IH as [k E]. exists (S k). simpl. rewrite E. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros A eqb H l1. induction l1 as [| x1 l1' IH1].
  - intros l2. destruct l2.
    + simpl. split. reflexivity. reflexivity.
    + simpl. split. intros H'. discriminate H'. intros H'. discriminate H'.
  - (*induction l2 as [| x2 l2' IH2].*)
    intros [| x2 l2'].
    + simpl. split. intros H'. discriminate H'. intros H'. discriminate H'.
    + simpl. split.
      * intros H'. rewrite andb_true_iff in H'. destruct H' as [H1' H2'].
        apply H in H1'. rewrite H1'.
        rewrite IH1 in H2'. rewrite H2'. reflexivity.
      * intros H'. injection H' as H1' H2'. rewrite andb_true_iff. split.
        -- apply H. apply H1'.
        -- apply IH1. apply H2'.
Qed.
...
Proof.
  ...
  { intros H. induction l as [|x' l' IH].
    - simpl. apply I.
    - simpl. simpl in H. rewrite andb_true_iff in H. destruct H as [H1 H2].
      apply IH in H2. apply (conj H1 H2). }
  { intros H. induction l as [|x' l' IH].
    - simpl. reflexivity.
    - simpl. apply andb_true_iff. simpl in H. destruct H as [H1 H2].
      apply IH in H2. apply (conj H1 H2). }
Qed.
...
Lemma cons_concat : forall {X: Type} (x: X) (l1 l2: list X), (x :: l1) ++ l2 = x :: (l1 ++ l2).
Proof.
  intros X x. induction l1 as [|x' l1' IH].
  - simpl. reflexivity.
  - simpl. reflexivity.
Qed.

Lemma rev_apped : forall {X: Type} (l l1 l2: list X), rev_append l (l1++l2) = (rev_append l l1) ++ l2.
Proof.
  intros X l. induction l as [|x t IH].
  - simpl. reflexivity.
  - simpl. intros l1 l2. rewrite <- cons_concat. apply IH.
Qed.
...
Proof.
  ...
  - simpl. unfold tr_rev. simpl. unfold tr_rev in IH. rewrite <- IH.
    rewrite <- rev_apped. simpl. reflexivity.
Qed.
(** [] *)
...
Theorem excluded_middle_irrefutable: forall (P : Prop),
  ~ ~ (P \/ ~ P).
Proof.
  intros P. unfold not. intros H. apply de_morgan_not_or in H.
  destruct H as [H1 H2]. apply H2. apply H1.
Qed.
...
Proof.
  intros Ex X P H x. specialize (Ex (P x)) as [H1|H2].
  - apply H1.
...
Proof.
  intros Base. unfold de_morgan_not_and_not. intros P Q H. destruct (Base P).
  - left. apply H0.
  - destruct (Base Q).
    + right. apply H1.
    + exfalso. apply H. apply (conj H0 H1).
Qed.

Theorem de_morgan_not_and_not__implies_to_or : de_morgan_not_and_not -> implies_to_or.
Proof.
  intros Base. unfold implies_to_or. intros P Q H. apply Base. intros [H1 H2].
  apply H1. intros H0. apply H2. apply H. apply H0.
Qed.

Theorem implies_to_or__excluded_middle : implies_to_or -> excluded_middle.
Proof.
  intros Base. unfold excluded_middle. intros P. specialize (Base P P). apply or_comm. apply Base. intros H. apply H.
Qed.

Theorem excluded_middle___peirce : excluded_middle -> peirce.
Proof.
  intros excluded_middle. unfold peirce. intros P Q.
  specialize (excluded_middle P) as [H|H].
  - intros _. apply H.
  - intros H'. apply H'. intros Hp. contradiction.
Qed.

Theorem peirce___consequentia_mirabilis : peirce -> consequentia_mirabilis.
Proof.
  intros peirce. unfold consequentia_mirabilis. intros P.
  specialize (peirce P False). apply peirce.
Qed.


Theorem consequentia_mirabilis__double_negation_elimination : consequentia_mirabilis -> double_negation_elimination.
Proof.
  intros Base. unfold double_negation_elimination. intros P H. apply Base. intros H'. contradiction.
Qed.

Theorem double_negation_elimination__excluded_middle : double_negation_elimination -> excluded_middle.
Proof.
  intros Base. unfold excluded_middle. intros P. specialize excluded_middle_irrefutable as H. apply Base. apply H.
Qed.



Theorem Six_Axioms_Are_Equivalent :
...
Proof.
  specialize excluded_middle__de_morgan_not_and_not as A.
  specialize de_morgan_not_and_not__implies_to_or as B.
  specialize implies_to_or__excluded_middle as C.
  specialize excluded_middle___peirce as D.
  specialize peirce___consequentia_mirabilis as E.
  specialize consequentia_mirabilis__double_negation_elimination as F.
  specialize double_negation_elimination__excluded_middle as G.
  repeat split; tauto.
Qed.
...
Proof.
  intros Base. unfold peirce. intros P Q H. apply H. intros Hp. apply Base in H. destruct H as [H|H].
Abort.
*)
(*
Theorem consequentia_mirabilis___implies_to_or : consequentia_mirabilis -> implies_to_or.
Proof.
  intros consequentia_mirabilis. unfold implies_to_or. intros P Q H.
  specialize (consequentia_mirabilis P) as H'.
Abort.
*)
(* FILL IN HERE
