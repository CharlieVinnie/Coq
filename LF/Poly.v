...
Proof.
  intros X l1 l2. induction l1 as [ | h t IHl1 ].
...
Proof.
  intros X l. induction l as [ | h t IHl ].
  - reflexivity. - simpl. rewrite rev_app_distr. rewrite IHl. reflexivity.
Qed.
(** [] *)
...
Definition filter_even_gt7 (l : list nat) : list nat := filter (fun x => even x && (7 <? x) ) l.
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. rewrite <- IHl. rewrite <- map_concat. reflexivity.
Qed.
(** [] *)
...
Definition fold_map {X Y: Type} (f: X -> Y) (l: list X) : list Y := fold (fun h l => (f h) :: l ) l [].
...
Proof.
  intros X Y Z f x y. unfold prod_curry. unfold prod_uncurry. simpl. reflexivity.
Qed.
...
Proof.
  intros X Y Z f p. unfold prod_curry. unfold prod_uncurry. destruct p. reflexivity.
Qed.
(** [] *)
...
Definition plus (n m : cnat) : cnat :=
  fun (X: Type) (f: X->X) (zero: X) => m X f (n X f zero).
...
Definition mult (n m : cnat) : cnat :=
  fun (X: Type) (f: X->X) (zero: X) => m X (n X f) zero.
