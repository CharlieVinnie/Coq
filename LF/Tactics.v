...
Proof.
  ...
  transitivity z. apply H3.
  assert ( y::l = z::l ). { rewrite H4. apply H2. }
  injection H as H'. symmetry. apply H'.
Qed.
(** [] *)
...
Proof.
  ...
  - intros m H. apply eqb_0_l in H. symmetry. apply H.
...
Proof.
  induction n as [ | n' IHn' ].
  - intros m H. simpl in H. destruct m. + reflexivity. + discriminate H.
...
Theorem nth_error_after_last: forall (n : nat) (X : Type) (l : list X),
  ...
  nth_error l n = None.
Proof.
  intros n X. induction n as [ | n' IHn' ].
  - intros l H. destruct l. + reflexivity. + discriminate H.
  - intros l H. destruct l as [ | h t ].
    + discriminate H.
    + simpl. simpl in H. injection H as H. apply IHn'. apply H.
Qed.
...
Proof.
  ...
  - intros l1 l2 H. simpl in H. injection H as H1 H2. rewrite <- H1. rewrite <- H2. reflexivity.
  - intros l1 l2 H. destruct h as [x y]. simpl in H. destruct (split t) as [lx ly].
    injection H as H1 H2. rewrite <- H1. rewrite <- H2. simpl.
    specialize IHl with lx ly. rewrite IHl. reflexivity. reflexivity.
Qed.
 (*
  intros X Y l l1 l2 H.
  generalize dependent l2.
  induction l1 as [ | h1 t1 IHl1 ].
  - intros l2 H. simpl. destruct l as [ | h t ].
    + simpl. reflexivity.
    + simpl in H. destruct h as [x y]. destruct (split t). discriminate H.
...
Proof.
  ...
  - destruct (f true) eqn: E2.
    + apply E2. + destruct b. * rewrite E1 in E2. discriminate E2. * apply E1.
  - destruct b.
    + destruct (f false) eqn: E2. apply E1. apply E2.
    + destruct (f false) eqn: E2. discriminate E1. apply E2.
Qed.
(** [] *)
...
Theorem eqb_trans : forall n m p,
  ...
  n =? p = true.
Proof.
  intros n m p H1 H2. generalize dependent n. generalize dependent p.
  induction m as [ | m' IHm' ].
  - intros p H1 n H2. destruct p,n.
    + reflexivity. + apply H2. + apply H1. + discriminate H1.
  - intros p H1 n H2. destruct p as [ | p'].
    + discriminate H1.
    + destruct n as [ | n'].
      * discriminate H2.
      * simpl in H1. simpl in H2. simpl. apply IHm'. apply H1. apply H2.
Qed.
(** [] *)
...
Proof.
  ...
  - intros l1 l2 H1 H2. destruct l1,l2. reflexivity. discriminate H1. discriminate H1. discriminate H2.
  - intros l1 l2 H1 H2. destruct l1 as [ | h1 t1 ].
    + discriminate H2.
    + destruct l2 as [ | h2 t2 ].
      * discriminate H1.
      * simpl in H1. injection H1 as H1. simpl in H2. injection H2 as H3 H4.
        specialize IHl with t1 t2. apply IHl in H1.
        rewrite <- H3. simpl. rewrite H1. reflexivity. apply H4.
Qed.
...
Theorem filter_exercise : forall (X : Type) (test : X -> bool)
  ...
  test x = true.
Proof.
  intros X test x l lf H. generalize dependent x.
  induction l as [ | h t IHl ].
  - intros x H. discriminate H.
  - intros x H. simpl in H. destruct (test h) eqn: E.
...
Proof.
  ...
  - simpl. unfold existsb'. simpl. destruct (test h) eqn: Eh.
    + simpl. reflexivity.
