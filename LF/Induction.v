...
Proof.
  ...
  - reflexivity.
  - rewrite -> IHn'. simpl.
    destruct (even n'). + reflexivity. + reflexivity.
Qed.
(** [] *)
...
Proof.
  intros b. simpl. destruct b as [ | b | b ].
  - simpl. reflexivity.
  - simpl. reflexivity.
  - simpl. reflexivity.
Qed.
...
Example normalize_test_1 : normalize (B0(B0 Z)) = Z.
Proof. reflexivity. Qed.

Example normalize_test_2 : normalize (B1(B0(B0 Z))) = B1 Z.
Proof. reflexivity. Qed.

Example normalize_test_3 : normalize (B1(B0(B1(B0 Z)))) = B1(B0(B1 Z)).
Proof. reflexivity. Qed.

Example normalize_test_4 : normalize (B1(B0(B1(B1 Z)))) = B1(B0(B1(B1 Z))).
Proof. reflexivity. Qed.
...
Lemma even_is_double : forall n: nat, nat_to_bin (n*2) = double_bin (nat_to_bin n).
Proof.
  induction n as [ | n IHn ].
  - reflexivity.
  - simpl. rewrite IHn.
    destruct (nat_to_bin n) as [ | m | m ].
    + reflexivity.
    + simpl. reflexivity.
    + simpl. reflexivity.
Qed.

Lemma incr_double_bin_is_adding_B1 : forall b: bin, incr (double_bin b) = B1 b.
Proof.
  intros [ | b | b ].
  - reflexivity.
  - simpl. reflexivity.
...
Proof.
  intros b. induction b as [ | b' IHb' | b' IHb' ].
  - simpl. reflexivity.
  - simpl. rewrite even_is_double. rewrite IHb'. reflexivity.
  - simpl. rewrite even_is_double. rewrite IHb'. rewrite incr_double_bin_is_adding_B1. reflexivity.
Qed.
